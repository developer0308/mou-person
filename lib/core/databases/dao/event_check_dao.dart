...
import '../table/event_checks.dart';

part 'event_check_dao.g.dart';

@DriftAccessor(tables: [EventChecks])
class EventCheckDao extends DatabaseAccessor<AppDatabase>
    with _$EventCheckDaoMixin {
  ...
  Future<EventCheck?> getEventCheckByDate(DateTime dateTime) =>
      (select(eventChecks)
            ..where((t) {
  ...
  Future updateEventCheck(EventCheck eventCheck) =>
      update(eventChecks).replace(eventCheck);

  Future deleteEventCheck(EventCheck eventCheck) =>
      delete(eventChecks).delete(eventCheck);
