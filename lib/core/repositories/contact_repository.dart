...
class ContactRepository {
  ...
  Future<Resource<List<Contact>>> getAllContact() async {
  ...
  Future<Resource<dynamic>> importContact(List<ContactRequest> contactRequests) {
    ...
    final resource = NetworkBoundResource<dynamic, dynamic>(
      ...
      saveCallResult: (c) => null,
    );
    return resource.getAsObservable();
  }

  Future<Resource<dynamic>> sendSMS(List<int> contactIDs) async {
    if (_cancelToken.isCancelled) _cancelToken = CancelToken();
    final resource = NetworkBoundResource<dynamic, dynamic>(
      createCall: () => APIService.sendSMS(contactIDs, _cancelToken),
      parsedData: (json) => {print(json)},
  ...
  Future<Resource<dynamic>> pushNotify(PushNotifyRequest request) async {
    return NetworkBoundResource<dynamic, dynamic>(
      createCall: () => APIService.pushNotify(request),
      parsedData: (json) => {print(json)},
    ).getAsObservable();
  }
